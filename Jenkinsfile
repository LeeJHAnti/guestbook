...
pipeline {
    ...
    environment {
        ...
        strDockerImage ="wow107/cicd_guestbook:${strDockerTag}"
    }

    stages {
        stage('Checkout') {
            steps {
                git branch: 'master', url:'https://github.com/LeeJHAnti/guestbook.git'

                dir('/root/sub-workspace/guestbook-config'){
                    git branch: 'master', url:'https://github.com/LeeJHAnti/guestbook-config.git'
                }
            }
        }
            ...
            steps {
                dir('/root/sub-workspace/guestbook-config'){
                    sh '''
                        ...
                        git push "https://LeeJHAnti:${GITHUB_ACCESS_TOKEN}@github.com/LeeJHAnti/guestbook-config.git"
                    '''
                }
            }
        }
